...
void BaseWaveApplLayer::initialize(int stage) {
	...
	if (stage==0) {
		...
		PkNumb = 1;
		PkNumberVec.setName("PacketNumber");
		XVec.setName("X_Pos");
		YVec.setName("Y_Pos");
		RxTimeVec.setName("RxTime");
		Rx_Beacons_Vec.setName("Rx_Beacons");
		beacon_Interval = par("beaconInterval").doubleValue();
		...
		double offSet = dblrand() * (beacon_Interval/2);
...
void BaseWaveApplLayer::handleSelfMsg(cMessage* msg) {
	switch (msg->getKind()) {
		case SEND_BEACON_EVT: {
			sendWSM(prepareWSM("beacon", beaconLengthBits, type_CCH, beaconPriority, 0, PkNumb));
			PkNumb = PkNumb + 1;
			scheduleAt(simTime() + beacon_Interval, sendBeaconEvt);
			break;
		}
		default: {
			if (msg)
