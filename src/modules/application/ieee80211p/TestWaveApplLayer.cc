...
void TestWaveApplLayer::initialize(int stage) {
	...
	receivedData = 0;
	PakNumber = 0;
	traci = TraCIMobilityAccess().get(getParentModule()->getParentModule());
}

void TestWaveApplLayer::onBeacon(WaveShortMessage* wsm) {
	receivedBeacons = receivedBeacons + 1;
	Rx_Beacons_Vec.record(receivedBeacons);
	PakNumber = wsm->getSerial();
	PkNumberVec.record(PakNumber);
//	MyPos = traci->getPositionAt(simTime());
	MyPos = curPosition;
	X = MyPos.x - 1030;
	Y = MyPos.y;
	XVec.record(X);
